...
// Cardinal heading (deg) that faces one voxel step (dx,dy).
...
void MappingAlgorithmImpl::ensureInitialized() {
    if (initialized_) {
        return;
    }
    initialized_ = true;
...
types::VoxelOccupancy MappingAlgorithmImpl::occAt(const geom::VoxelIndex& idx) const {
    return output_map_.atVoxel(geom::voxelCenter(idx, cfg_));
}

bool MappingAlgorithmImpl::inGrid(const geom::VoxelIndex& idx) const {
    return idx.x >= 0 && idx.x < nx_ && idx.y >= 0 && idx.y < ny_ && idx.z >= 0 && idx.z < nz_;
}

bool MappingAlgorithmImpl::hasClearance(const geom::VoxelIndex& idx) const {
    if (clearance_ <= 0) {
        return true;
    }
...
bool MappingAlgorithmImpl::traversable(const geom::VoxelIndex& idx) const {
    return inGrid(idx) && occAt(idx) == types::VoxelOccupancy::Empty && hasClearance(idx);
}

bool MappingAlgorithmImpl::isFrontier(const geom::VoxelIndex& idx) const {
...
bool MappingAlgorithmImpl::anyUnmappedRemains() const {
...
std::vector<geom::VoxelIndex> MappingAlgorithmImpl::planPath(const geom::VoxelIndex& start) const {
...
void MappingAlgorithmImpl::buildMoveQueue(const types::DroneState& state,
