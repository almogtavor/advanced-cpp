...
[[nodiscard]] common::types::MappingBounds parseBounds(const YAML::Node& node, const std::string& context) {
...
[[nodiscard]] common::types::DroneConfigData parseDroneNode(const YAML::Node& root) {
...
[[nodiscard]] common::types::LidarConfigData parseLidarNode(const YAML::Node& root) {
...
[[nodiscard]] common::types::MissionConfigData parseMissionNode(const YAML::Node& root) {
...
common::types::DroneConfigData parseDroneConfig(const std::string& yaml_text) {
    return parseDroneNode(YAML::Load(yaml_text));
}
common::types::LidarConfigData parseLidarConfig(const std::string& yaml_text) {
    return parseLidarNode(YAML::Load(yaml_text));
}
common::types::MissionConfigData parseMissionConfig(const std::string& yaml_text) {
...
common::types::DroneConfigData loadDroneConfig(const std::filesystem::path& file) {
    return parseDroneNode(YAML::LoadFile(file.string()));
}
common::types::LidarConfigData loadLidarConfig(const std::filesystem::path& file) {
    return parseLidarNode(YAML::LoadFile(file.string()));
}
common::types::MissionConfigData loadMissionConfig(const std::filesystem::path& file) {
...
types::SimulationCompositionData loadComposition(const std::filesystem::path& file) {
    ...
    for (const YAML::Node& d : require(comp, "drone_configs", "simulation_compositions")) {
        data.drone_configs.push_back(loadDroneConfig(resolve(d.as<std::string>())));
    }
    for (const YAML::Node& l : require(comp, "lidar_configs", "simulation_compositions")) {
        data.lidar_configs.push_back(loadLidarConfig(resolve(l.as<std::string>())));
    }
    return data;
}
