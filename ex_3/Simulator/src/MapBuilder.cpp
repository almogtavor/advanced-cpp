...
ResolutionDecision decideResolution(const common::types::MissionConfigData& mission) {
...
common::types::MapConfig makeOutputMapConfig(const common::types::MissionConfigData& mission) {
    const ResolutionDecision decision = decideResolution(mission);
    const common::types::MappingBounds& b = mission.mission_bounds;
...
common::types::MapConfig makeHiddenMapConfig(const types::SimulationConfigData& sim,
    ...
    config.boundaries = common::types::MappingBounds{
...
std::shared_ptr<NpyArray> loadHiddenMapArray(const std::filesystem::path& path) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        data[i] = (data[i] != 0)
                      ? static_cast<std::int8_t>(common::types::VoxelOccupancy::Occupied)
                      : static_cast<std::int8_t>(common::types::VoxelOccupancy::Empty);
    }
    return arr;
}

std::shared_ptr<NpyArray> makeOccupancyGrid(const common::types::MapConfig& config,
                                            common::types::VoxelOccupancy fill) {
    const double res = geom::lcm(config.resolution);
    const common::types::MappingBounds& b = config.boundaries;
