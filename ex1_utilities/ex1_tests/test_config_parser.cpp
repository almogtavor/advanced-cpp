...
max_elevate_per_cmd 50
    ...
    CHECK(r.ok);
    CHECK_NEAR(cfg.min_passage_width.in_cm(), 50.0, 1e-9);
    CHECK_NEAR(cfg.lidar_fov.in_deg(), 90.0, 1e-9);
    CHECK_NEAR(cfg.lidar_max_range.in_cm(), 400.0, 1e-9);
    CHECK_NEAR(cfg.max_advance_per_cmd.in_cm(), 50.0, 1e-9);
    std::remove(p.c_str());
}
...
lidar_max_range 200
    ...
    CHECK_NEAR(cfg.lidar_fov.in_deg(), 60.0, 1e-9);
    CHECK_NEAR(cfg.lidar_max_range.in_cm(), 200.0, 1e-9);
    std::remove(p.c_str());
}
...
polygon_vertex 0 100
    ...
    CHECK_NEAR(m.start.x.in_cm(), 50.0, 1e-9);
    CHECK_NEAR(m.start.y.in_cm(), 50.0, 1e-9);
    CHECK_NEAR(m.height_max.in_cm(), 100.0, 1e-9);
    std::remove(p.c_str());
}
